...
void sym_mat3::eigenvectors(double &eig1,double &eig2,double &eig3,mat3 &Lam) {
    ...
    if(nonzero(eig1-eig2,t)) {
        ...
        // If the eigenvalues are distinct then just call the regular routine
        // to find independent eigenvalues
        ...
        // If the eigenvalues are equal, then the matrix has rank of at most
        // one; since it is symmetric this gives it a very specific form. Use
        // this to calculate two orthogonal eigenvectors
        double b11=a11-eig1;
        if(nonzero(b11,t)) {
            if(nonzero(a12,t)) {
